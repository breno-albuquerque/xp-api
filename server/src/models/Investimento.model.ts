...
class InvestimentoModel {
    ...
    const [result] = await conn
      .run(conn.qInvestimento.getOne, [CodCliente, CodAtivo]) as RowDataPacket[];
  ...
  public static async create(conn: IConnection, investment: IInvestimento): Promise<number> {
    ...
    const result = await conn
      .run(conn.qInvestimento.create, [CodCliente, CodAtivo, QtdeAtivo]) as ResultSetHeader;
  ...
  public static async update(conn: IConnection, investment: IInvestimento): Promise<number> {
    ...
    const result = await conn
      .run(conn.qInvestimento.update, [QtdeAtivo, CodAtivo, CodCliente]) as ResultSetHeader;
    return result.affectedRows;
  }

  public static async delete(conn: IConnection, assetId: number, contaId: number): Promise<number> {
    const result = await conn
      .run(conn.qInvestimento.delete, [assetId, contaId]) as ResultSetHeader;
    return result.affectedRows;
  }
}
